...
describe('ifx-date-picker', () => {
    ...
    it('should update the input value when a date is selected', async () => {
        ...
        await page.$eval('ifx-date-picker', (el, value) => {
            const input = el.shadowRoot.querySelector('.date__picker-input');
            input.value = value;
            input.dispatchEvent(new Event('change'));
        }, dateString);
        const inputValue = await page.$eval('ifx-date-picker >>> .date__picker-input', (el) => el.value);
...
async function addHandleSubmitScript(page) {
...
async function submitAndGetValue(page) {
